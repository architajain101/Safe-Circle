...
async function executeAI(modelName: string, parameters: any) {
  ...
  try {
    ...
    if (
      ...
      console.warn("Primary key quota exhausted.");
...
Output: JSON only.`;
    ...
    if (
      !["consistent", "inconsistent", "verified"].includes(
        result.consistencyFlag
      )
    ) {
      result.consistencyFlag = "consistent";
    }
...
export async function analyzeReportAdvanced(
  userText: string,
  category: string,
  imageUrl?: string,
  audioUrl?: string
...
Analyze context and sentiment. Determine threat level.
Perform OCR and transcription if media exists.`;
    ...
    if (
      !["low", "medium", "high", "critical"].includes(result.threatLevel)
    ) {
